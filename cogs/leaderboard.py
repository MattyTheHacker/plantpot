...
class Leaderboard(commands.Cog):
    ...
    async def getlb(self, ctx, m):
        ...
        for i in range(x):
            ...
            lbtxt += '{0}. **{1}** - {2}\n'.format(
                i+1, uid.display_name, lb[i]['points'])
        embed = discord.Embed(
            colour=ctx.guild.get_member(self.bot.user.id).colour)
                ...
                for i in range(len(lb)):
                    if lb[i]['userid'] == ctx.message.author.id:
                        embed = discord.Embed(
                            colour=ctx.guild.get_member(self.bot.user.id).colour)
                        embed.title = f'you are in #{i+1} place!'
                        embed.description = 'you have collected {0} characters so far, totalling {1} points! keep it up!'.format(
                            len(lb[i]['images']), lb[i]['points'])
                        embed.set_thumbnail(
                            url=ctx.message.author.avatar_url_as())
                        if m is None:
                            return await ctx.send(embed=embed)
            ...
            if await Leaderboard.checkuser(self, username.id, d):
                ...
                for i in range(len(lb)):
                    if lb[i]['userid'] == username.id:
                        embed = discord.Embed()
                        embed.title = f'{username.display_name} is #{i+1} place!'
                        embed.description = 'they have collected {0} characters so far, totalling {1} points!'.format(
                            len(lb[i]['images']), lb[i]['points'])
                        embed.set_thumbnail(url=username.avatar_url_as())
                        if m is None:
                            return await ctx.send(embed=embed)
